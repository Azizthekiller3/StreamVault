...
import { Search as SearchIcon, Play, Heart, Sun, Moon } from "lucide-react";
...
import { isInWatchlist, toggleWatchlist } from "@/lib/flixnest-store";
...
function MovieCard({ movie, index, isNew, onWatchlistChange }: {
  movie: TelegramMovie;
  index: number;
  isNew: boolean;
  ...
  return (
    ...
    >
      <div className="relative aspect-[2/3] rounded-md overflow-hidden bg-white/5">
        ...
        {/* NEW badge */}
        {isNew && (
          <div className="absolute top-1 left-1 bg-green-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide">
            NEW
          </div>
        )}
        {/* Watchlist heart */}
        ...
        {genres.length > 0 && (
          <div className="absolute top-6 left-1 flex flex-col gap-0.5">
            {genres.map((g) => (
              <span key={g.genre} className="text-[7px] bg-black/70 text-white/90 px-1 rounded font-medium">{g.emoji}</span>
            ))}
          </div>
        )}
        {/* Quality badges */}
...
export default function Home() {
  ...
  const handleThemeToggle = () => {
    ...
    if (isDark) {
      ...
      localStorage.setItem("flixnest_theme", "light");
      setIsDark(false);
    } else {
      html.classList.replace("light", "dark");
      localStorage.setItem("flixnest_theme", "dark");
      setIsDark(true);
    }
  };
  ...
  if (telegramSearch.trim()) {
    filtered = filtered.filter((m) => m.title.toLowerCase().includes(telegramSearch.toLowerCase()));
  }
  if (activeGenre) {
    filtered = filtered.filter((m) => detectGenres(m.title).some((g) => g.genre === activeGenre));
  }

  const visible = filtered.slice(0, visibleCount);
  const hasMore = filtered.length > visibleCount;

  // Genres that actually appear in the movie list
  const availableGenres = ALL_GENRES.filter((g) =>
    allMovies.some((m) => detectGenres(m.title).some((dg) => dg.genre === g.genre))
  );

  const isSearchActive = telegramSearch.trim() || activeGenre;
    ...
    <div className="bg-background min-h-screen pb-4">
      ...
      <div className="relative w-full bg-black" style={{ height: "52vw", minHeight: 200, maxHeight: 320 }}>
        {heroItem?.poster && heroItem.poster !== "N/A" ? (
          <>
            <img src={heroItem.poster} alt={heroItem.title} className="absolute inset-0 w-full h-full object-cover opacity-50" />
        ...
        <section>
          ...
          <div className="flex items-center gap-2 mb-2 px-4">
            ...
            <div className="relative flex-1">
              ...
              <input
                ...
                onChange={(e) => { setTelegramSearch(e.target.value); setVisibleCount(20); }}
          ...
          {availableGenres.length > 0 && (
            <div className="flex gap-2 overflow-x-auto pb-2 px-4 scrollbar-hide mb-3">
              <button
                onClick={() => { setActiveGenre(null); setVisibleCount(20); }}
              ...
              {availableGenres.map((g) => (
                <button
                  key={g.genre}
                  onClick={() => { setActiveGenre(activeGenre === g.genre ? null : g.genre); setVisibleCount(20); }}
          ...
          {isTelegramLoading && (
            <div className="grid grid-cols-3 gap-2 px-4 pb-3">
              {Array.from({ length: 6 }).map((_, i) => (
                <Skeleton key={i} className="aspect-[2/3] rounded-md bg-white/10" />
              ))}
            </div>
          )}
          ...
          {!isTelegramLoading && isSearchActive && filtered.length === 0 && (
            <p className="px-4 text-white/40 text-sm py-4">No movies found. Try a different search or genre.</p>
          )}
              ...
              <div className="flex gap-2.5 overflow-x-auto pb-3 pl-4 pr-4 scrollbar-hide snap-x">
                {allMovies.slice(0, 20).map((movie, i) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    transition={{ delay: i * 0.03 }}
                    className="flex-none w-[110px] snap-start cursor-pointer relative"
                    onClick={() => setLocation(`/telegram-info?id=${movie.id}`)}
                  >
                    <div className="relative aspect-[2/3] rounded-md overflow-hidden bg-white/5">
                      {movie.poster ? (
                      ...
                      {/* NEW badge for first 8 */}
                      {i < 8 && (
                        <div className="absolute top-1 left-1 bg-green-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full uppercase">NEW</div>
                      )}
          ...
          {/* Filtered grid (search or genre active) */}
          {!isTelegramLoading && isSearchActive && filtered.length > 0 && (
            <>
              <div className="grid grid-cols-3 gap-2 px-4 pb-3">
                {visible.map((movie, i) => (
                  <MovieCard
                    ...
                    movie={movie}
                    index={i}
                    isNew={parseInt(movie.id) > maxMsgId - 50}
                    onWatchlistChange={() => setWatchlistTick(t => t + 1)}
                  />
                ))}
              </div>
              {hasMore && (
          ...
          {!isTelegramLoading && !isSearchActive && allMovies.length > 20 && (
            <div className="px-4 pb-2">
              <button
                onClick={() => { setTelegramSearch(" "); setTimeout(() => setTelegramSearch(""), 0); setVisibleCount(allMovies.length); setActiveGenre(null); }}
                className="w-full py-2.5 rounded-xl bg-white/10 border border-white/10 text-white/70 text-sm font-semibold hover:bg-white/20 transition-colors"
              >
        ...
        {history && history.length > 0 && (
          <section>
            <div className="flex items-center justify-between mb-3 px-4">
              ...
              <Link href="/history">
                <button className="text-white/50 text-sm hover:text-white transition-colors">more</button>
              </Link>
            </div>
            <div className="flex gap-2.5 overflow-x-auto pb-3 pl-4 pr-4 scrollbar-hide snap-x">
